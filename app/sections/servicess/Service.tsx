...
export default function Service({ icon, title, text }: Props) {
  ...
  return (
    <div onClick={() => setShowText(!showText)}>
      <Container className={`
        px-4 py-5 
        w-full ${showText ? "h-[230px]" : "h-[68px]"} 
      ...
      `}>
        ...
        />
        <h3 className="font-bold mb-5 self-center !text-[18px]  md:!text-[2vw] lg:!text-[1.5vw] xl:!text-[20px] text-center leading-[20px] mt-2 ">{title}</h3>
        <p className="!text-[16px] md:!text-[1.6vw] lg:!text-[1.4vw] xl:!text-[16px]">{text}</p>
        <Plus className={`
